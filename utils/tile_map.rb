...
class TileMap < Chingu::BasicGameObject
  ...
  class TileSet
    ...
    TileSet.autoload_dirs = [ File.join("media")]
    ...
    def initialize options
      ...
      @tiles = @tids.each_with_object({}) do |x, tiles|
        next if x == 0
        index = x - 1

        tile = sets[index][:tiles]

        tiles[x] = { :image => tile.first, :name => sets[index][:name] }
      end
    end
    ...
    def get_tile(tile_no)
      @tiles[tile_no][:image] ? @tiles[tile_no][:image] : fail("tile #{tile_no} does not exist in map #{@name}")
    end

    def get_name(tile_no)
      @tiles[tile_no][:name] ? @tiles[tile_no][:name] : fail("tile #{tile_no} does not exist in map #{@name}")
    end
  end
  ...
  class Tile
    ...
    def set_info type, tileset
      ...
      if type == 0
        ...
        @image = tileset.get_tile(type)
        @name  = tileset.get_name(type)
      end
    end
